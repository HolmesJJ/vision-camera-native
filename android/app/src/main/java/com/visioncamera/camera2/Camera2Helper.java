        ...
        try {
          for(int y = 0; y < height; ++y) {
            ...
            for (int x = 0; x < width; ++x) {
              nv21Bytes[idY++] = yBuffer.get(yOffset + x * yPixelStride);
              if (y < uvHeight && x < uvWidth) {
                int bufferIndex = uvOffset + (x * uvPixelStride);
                // V channel.
                nv21Bytes[idUV++] = vBuffer.get(bufferIndex);
                // U channel.
                nv21Bytes[idUV++] = uBuffer.get(bufferIndex);
              }
            }
          }
          camera2Listener.onPreview(nv21Bytes, mPreviewSize);
        } catch (Exception e) {
          e.printStackTrace();
        }
        lock.unlock();
